...
@property(strong,nonatomic)EUExImage * slectImage;
@end
...
- (void)sheetViewDidSelectIndex:(NSInteger)Index selectTitle:(NSString *)title
{
    ...
    [self performSelector:@selector(delayMethods:) withObject:[NSString stringWithFormat:@"%ld",(long)Index] afterDelay:0.3];
    
}
-(void)delayMethods:(NSString*)indexStr
{
    UexImageMySingleton * shareMY = [UexImageMySingleton shareMySingLeton];
    
    NSString *jsStr = [NSString stringWithFormat:@"if(uexImage.cbDidChooseFunction!=null){uexImage.cbDidChooseFunction('%@');}",indexStr ];
    
    [shareMY.slectImage.meBrwView stringByEvaluatingJavaScriptFromString:jsStr];
}
